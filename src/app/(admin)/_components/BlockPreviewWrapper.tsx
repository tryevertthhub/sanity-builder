...
import { Code, Zap, Sliders, RotateCcw } from "lucide-react";
import { Block } from "../types";
import { BLOCK_COMPONENTS, BLOCKS } from "@/src/components/blocks";
...
// Function to extract initial values from a block schema
const getBlockInitialValues = (blockType: string): Partial<Block> => {
  const blockConfig = BLOCKS[blockType as keyof typeof BLOCKS];
  if (!blockConfig?.schema) {
    console.warn(`No schema found for block type: ${blockType}`);
    return {};
  }

  const schema = blockConfig.schema;
  const initialValues: any = {
    _type: blockType,
  };

  // Extract initial values from schema fields
  if (schema.fields) {
...
export function BlockPreviewWrapper({
  ...
  const handleReset = React.useCallback(() => {
    ...
    const initialValues = getBlockInitialValues(blockType);
    const resetBlock = {
      ...block, // Keep id, _key, etc.
      ...initialValues, // Apply initial values
    };

    setLocalBlock(resetBlock);
    setIsDirty(true);

    // Persist the reset
    if (onUpdate && block.id) {
      Promise.resolve().then(() => onUpdate(block.id, resetBlock));
    }
  }, [block, onUpdate]);
        ...
        <>
          ...
          {isEditMode && (
            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button
                onClick={handleReset}
                className="p-1.5 rounded-lg bg-zinc-800/80 hover:bg-zinc-700/80 text-zinc-400 hover:text-orange-400 transition-colors ml-1"
